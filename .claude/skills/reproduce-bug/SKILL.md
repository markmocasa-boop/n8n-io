---
name: reproduce-bug
description: Reproduce a bug from a Linear ticket with a failing test. Takes a ticket ID or URL.
user_invocable: true
---

# Bug Reproduction Framework

Given a Linear ticket ID or URL ($ARGUMENTS), systematically reproduce the bug
with a failing regression test.

## Step 1: Parse Signals

Fetch the Linear issue using `mcp__linear__get_issue` and extract:
...
If the ticket links to a GitHub issue, fetch that too with `gh issue view`.
